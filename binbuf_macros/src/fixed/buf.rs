...
pub fn output(mut input: Input, lib: &syn::Path) -> TokenStream {
    let ptr_trait_bound: syn::TraitBound = syn::parse2(quote! { #lib::fixed::Ptr }).unwrap();
    ...
    quote! {
        ...
        impl #generics_params #lib::fixed::Readable<#entry_ty> for #ident #ty_generics {
            fn write_to(self, buf: #lib::fixed::BufMut<#entry_ty>) {
                #lib::fixed::buf_copy_to::<#entry_ty>(#ident(self.0.to_const(), ::std::marker::PhantomData), buf);
            }
        }
    }
}
