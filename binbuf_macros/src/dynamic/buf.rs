...
pub fn output(mut input: Input, lib: &syn::Path) -> TokenStream {
    let ptr_trait_bound: syn::TraitBound = syn::parse2(quote! { #lib::dynamic::Ptr }).unwrap();
    ...
    quote! {
        ...
        impl #generics_params #lib::dynamic::Readable<#entry_ty> for #ident #ty_generics {
            fn write_to(self, buf: #lib::dynamic::BufMut<#entry_ty>) -> usize {
                #lib::dynamic::buf_copy_to::<#entry_ty>(#ident(self.0.to_const(), ::std::marker::PhantomData), buf)
            }
            fn len(&self) -> usize {
                #lib::dynamic::buf_len::<#entry_ty>(#lib::dynamic::buf_to_const::<#entry_ty, _>(*self))
            }
        }
    }
}
