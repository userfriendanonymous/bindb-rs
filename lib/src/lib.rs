...
use memmap::Mmap;
pub use lens::{Value as Lens, RootProducer as RootLenser};
...
fn benchmarks() {
    ...
    let item = benchmark("find", || {
        db.find(Item::lenser().bar(), |&x| x >= total - 10).unwrap()
    });
...
struct ItemLenser(RootLenser<Item>);
...
impl ItemLenser {
    ...
    fn foo(&self) -> Lens<Item, Option<f32>> {
        self.0.spawn(0)
    }

    fn bar(&self) -> Lens<Item, u64> {
        self.0.spawn(<Option<f32>>::SIZE)
    }
}
